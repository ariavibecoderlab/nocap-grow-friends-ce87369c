...
serve(async (req) => {
  ...
  try {
    ...
    const { data: allowed } = await supabase.rpc('check_rate_limit', {
      p_identifier: apiKey, p_endpoint: 'api-referral-network', p_max_requests: 30, p_window_seconds: 60,
    });
    if (!allowed) {
    ...
    for (const r of (referrals || [])) {
      ...
      tierMap[r.tier].push({
        ...
        joined: r.created_at?.split('T')[0] || '',
      });
    }

    const tiers = [];
    for (let t = 1; t <= 5; t++) {
      const members = tierMap[t] || [];
      if (members.length > 0) {
        tiers.push({ tier: t, count: members.length, members });
      }
    }
    ...
    try {
      await supabase.from('api_request_logs').insert({
        app_id: app.id, endpoint: '/api-referral-network', method: 'GET', status_code: 200,
        request_body: {}, response_body: { tiers_count: tiers.length }, user_id: token.user_id,
