...
export function generateApiGuidePdf() {
  ...
  subheading("Prompt 14 — Webhook Replay with Idempotency");
  paragraph("(Reliability upgrade — for any integrator already consuming webhooks) Merchant Action: None. Endpoint: POST /api-webhooks-replay with headers x-api-key, x-api-secret, content-type: application/json, and Idempotency-Key (UUID v4, max 255 chars, recommended). Body: { delivery_id: <UUID of the original webhook_deliveries row> }.");
  paragraph("Behavior: The server reloads the original delivery (must belong to your app — others return 404), re-serializes the stored payload byte-for-byte, recomputes HMAC-SHA256 with your current api_secret_hash, and refuses to dispatch (409) if the recomputed signature does not match the original recorded signature (e.g., secret rotated since first dispatch). On success it POSTs the exact original payload to your current webhook_url with the same X-Webhook-Signature, X-Webhook-Event headers, then writes a new webhook_deliveries row tagged status='replayed' and replayed_from_id=<original_id>.");
  paragraph("Idempotency: Repeating the same Idempotency-Key within 24h returns the cached response with header Idempotent-Replay: true (no duplicate dispatch). Reusing the same key with a different body returns 409 'Idempotency-Key reused with different request body'. Always generate a fresh UUID for genuinely new replay intents and persist the (idempotency_key, original_delivery_id, last_response) tuple locally.");
  paragraph("Rate limits: 30 replays/min per merchant and 5 replays/min per delivery_id. On 429, honor retry_after_seconds before retrying. On 5xx, retry with exponential backoff (1s, 2s, 4s) keeping the SAME Idempotency-Key. Error matrix: 400 invalid body, 401 invalid creds, 404 delivery not found / not yours, 409 signature integrity or idempotency conflict, 429 rate limited.");
  paragraph("Implementation: 1) Add a 'Replay' button on your webhook delivery log UI calling this endpoint. 2) Generate Idempotency-Key per click (crypto.randomUUID). 3) Store the response keyed by (delivery_id, idempotency_key). 4) On success, surface the new replay_id and update the delivery row to show it was replayed. 5) Treat Idempotent-Replay:true responses as success — do not show 'duplicate' errors to the user.");
  paragraph("Member Impact: None — operational tooling only. Improves merchant trust by guaranteeing recoverable webhook delivery without double-processing risk.");
