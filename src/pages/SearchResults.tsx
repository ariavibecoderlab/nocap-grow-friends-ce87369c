...
export default function SearchResults() {
  ...
  // Debounce timer for search input changes
    ...
    async (
      ...
      try {
        ...
        if (append) {
          ...
          setTotalCount(
            rows.length > 0
              ? (rows[0] as unknown as { total_count?: number }).total_count ??
                  rows.length
              : 0
          );
        }
  ...
  useEffect(() => {
    setOffset(0);
    setProducts([]);
    fetchProducts(query, sort, inStock, minPrice, maxPrice, 0, false);
  }, [query, sort, inStock, minPrice, maxPrice, fetchProducts]);
  ...
  const handleLoadMore = () => {
    fetchProducts(query, sort, inStock, minPrice, maxPrice, offset, true);
  };

  const applyPriceFilter = () => {
    setPriceOpen(false);
    // The useEffect on minPrice/maxPrice will fire automatically
  };
      ...
      <div className="mx-auto max-w-md px-4">
        ...
        <div className="flex items-center gap-2 py-3 overflow-x-auto scrollbar-hide">
          ...
          <Select value={sort} onValueChange={(v) => setSort(v as SortOption)}>
            <SelectTrigger className="h-8 shrink-0 bg-white/5 border-white/10 text-white text-xs w-40">
        ...
        {!loading && query && (
          <p className="text-[11px] text-white/40 mb-3">
            {products.length > 0
              ? `${
                  totalCount > products.length ? totalCount : products.length
          ...
          <>
            ...
            {!hasMore && products.length >= PAGE_SIZE && (
              <p className="text-center text-[10px] text-white/20 py-4">
                All {products.length} results shown
              </p>
            )}
          </>
        )}
