...
import {
  shareWithdrawalReceipt,
  downloadWithdrawalReceipt,
} from "@/lib/generateWithdrawalReceiptPdf";
...
const Withdraw = () => {
  ...
  const fetchData = async () => {
    if (!user) return;
    setLoading(true);
    const [{ data: wr }, { data: wallet }, { data: globalSettings }] =
      await Promise.all([
        supabase
  ...
  // Block new requests only while one is pending or actively being transferred
  ...
  const submit = async () => {
    ...
    if (amt < minWithdrawal) {
      toast({
        title: `Minimum withdrawal is RM ${minWithdrawal.toFixed(2)}`,
        variant: "destructive",
      });
      return;
    }
    ...
    if (error || !(result as any)?.success) {
      ...
      if ((result as any)?.new_balance != null)
    ...
    <div className="min-h-screen bg-primary pb-20">
          ...
          requests.map((r) => (
            ...
            >
              <CardContent className="p-3">
                ...
                {r.status === "processing" && (
                  <p className="text-[10px] text-blue-400 mt-1.5 bg-blue-500/10 rounded px-2 py-1">
                    💸 Your transfer is on the way — usually arrives within 1
                    business day.
                  </p>
                )}
                {r.status === "failed" && (
                  <p className="text-[10px] text-red-400 mt-1.5 bg-red-500/10 rounded px-2 py-1">
                    Transfer failed. Our team has been notified and will process
                    this manually.
                  </p>
                )}
                {r.rejection_reason && (
        ...
        <DialogContent className="max-w-sm bg-primary border-white/10">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              />
              <p className="text-[10px] text-white/40">
                Min: RM {minWithdrawal.toFixed(2)} • Available: RM{" "}
      ...
      >
        <DialogContent className="border-white/10 bg-primary text-white max-w-sm mx-auto">
          <DialogHeader>
            <DialogTitle className="font-display text-lg text-white text-center">
              Withdrawal Details
            </DialogTitle>
          </DialogHeader>
          {selectedRequest &&
              ...
              return (
                <div className="space-y-5">
                  ...
                  <div className="space-y-3">
                    <DetailRow
                      label="Date"
                      value={date.toLocaleDateString("en-MY", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                      })}
                    ...
                    <DetailRow
                      label="Time"
                      value={date.toLocaleTimeString("en-MY", {
                        hour: "2-digit",
                        minute: "2-digit",
                        second: "2-digit",
                        hour12: true,
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={async () => {
                        setSharing(true);
                        try {
                          await shareWithdrawalReceipt(r);
                        } catch {
                          /* cancelled */
                        }
                        setSharing(false);
...
const DetailRow = ({
  ...
  <div className="flex items-start justify-between gap-4">
    ...
    <span
      className={`text-xs text-white text-right break-all ${mono ? "font-mono text-[10px]" : ""}`}
    >
      {value}
    </span>
  </div>
