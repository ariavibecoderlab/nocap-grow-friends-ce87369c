...
import MerchantNavigation, {
  getMerchantSections,
} from "@/components/merchant/MerchantNavigation";
  ...
  if (!storeId)
    return (
      <div className="text-center text-white/40 py-12 text-sm">
        No store found for this branch. Create a store in the Shop tab first.
      </div>
    );
...
const MerchantNavigationWrapper = ({
  selectedBranch,
  branches,
  user,
  chatUnread,
  ...
  const handleTabChange = (value: string) => {
    ...
    if (HUB_TABS.has(value)) {
      ...
      navigate(
        `/merchant/storefront?section=${sectionMap[value] || "overview"}`
      );
      return;
    }
    setActiveTab(value);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "chat":
        return <MerchantChatTab branchId={selectedBranch.id} />;
      case "txns":
        return (
      ...
      case "withdraw":
        return (
          <MerchantWithdrawals
            ...
            branchBalance={Number((selectedBranch as any)?.balance || 0)}
      ...
      case "reports":
        return <MerchantSettlement userId={user.id} branches={branches} />;
      case "analytics":
        return <MerchantAnalytics userId={user.id} branches={branches} />;
      case "sales":
        return (
      ...
      case "api":
        return <MerchantApiApps branches={branches} />;
      case "logs":
        return <MerchantApiLogs />;
      case "webhooks":
        return <MerchantWebhookDeliveries />;
      default:
        return null;
    }
  };
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex items-center gap-2 border-b border-white/10 px-4 py-2 bg-primary">
            ...
            <span className="text-xs text-white/30 truncate">
              {getMerchantSections(chatUnread)
                .flatMap((s) => s.items)
                .find((i) => i.value === activeTab)?.label || "Dashboard"}
            </span>
          </div>
...
const MerchantDashboard = () => {
  ...
  const [onboardingDismissed, setOnboardingDismissed] = useState(
    () => sessionStorage.getItem("merchant-onboarding-dismissed") === "true"
  );
  ...
  // QR display dialog
  const [showQrDisplay, setShowQrDisplay] = useState<{
    type: "static" | "dynamic";
    data: string;
    label: string;
  } | null>(null);
        ...
        (payload) => {
          ...
          if (updated.is_used) {
            toast({
              ...
              description: `RM ${Number(updated.amount).toFixed(2)} paid`,
            });
          }
        }
      )
  ...
  const createDynamicQr = async () => {
    ...
    if (error) {
      ...
      const qrData = JSON.stringify({
        branch_id: selectedBranch.id,
        qr_id: (data as DynamicQr).id,
      });
      setShowQrDisplay({
        type: "dynamic",
        data: qrData,
        label: `RM ${Number(qrAmount).toFixed(2)}`,
      });
    }
    setCreatingQr(false);
  };
  ...
  const showDynamicQrCode = (qr: DynamicQr) => {
    ...
    setShowQrDisplay({
      type: "dynamic",
      data: qrData,
      label: `RM ${Number(qr.amount).toFixed(2)}`,
    });
  };
          ...
          <Card className="border-white/10 bg-white/5 shadow-2xl backdrop-blur">
            <CardContent className="flex flex-col items-center py-12">
              ...
              <p className="mt-2 text-sm text-white/50 text-center max-w-xs">
                Register as a merchant to start accepting payments and manage
                your business.
              </p>
              <Button
  ...
  const statsBlock = (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <Card className="border-white/10 bg-white/5">
        <CardContent className="p-4 text-center">
          <Wallet className="mx-auto h-4 w-4 text-secondary" />
          <p className="mt-1 font-display text-xl font-bold text-white">
            {formatRM((selectedBranch as any)?.balance)}
          </p>
      ...
      <Card className="border-white/10 bg-white/5">
        <CardContent className="p-4 text-center">
          <Store className="mx-auto h-4 w-4 text-secondary" />
          <p className="mt-1 font-display text-xl font-bold text-white">
            {branches.length}
          </p>
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          className="gap-1 border-white/10 text-white/70 hover:bg-white/10 hover:text-white"
        >
      ...
      {branches.length === 0 ? (
        <Card className="border-white/10 bg-white/5">
          <CardContent className="flex flex-col items-center py-8 text-white/40">
            ...
            <p className="text-xs mt-1">
              Add your first branch to start receiving payments
            </p>
          </CardContent>
        </Card>
      ) : (
        <div
          className={
            isMobileView ? "space-y-2" : "grid grid-cols-1 md:grid-cols-2 gap-3"
          }
        >
          {branches.map((b) => (
            <Card
              ...
              className={`border-white/10 bg-white/5 cursor-pointer transition-all ${
                selectedBranch?.id === b.id ? "ring-2 ring-secondary" : ""
              }`}
      ...
      <Card className="border-secondary/20 bg-secondary/10">
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            ...
            <Button
              size="sm"
              onClick={() => showStaticQr(selectedBranch!)}
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            {editingCommission ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (isNaN(val) || val < 0 || val > 20) {
                      toast({
                        title: "Invalid value",
                        description: "Commission must be between 0% and 20%",
                        variant: "destructive",
                      });
                      return;
                    }
              ...
              <div className="flex items-center gap-2">
                <span className="font-medium text-white">
                  {selectedBranch?.commission_percent}%
                </span>
                <Button
                  size="icon"
                  variant="ghost"
                  ...
                  onClick={() => {
                    setCommissionValue(
                      String(selectedBranch?.commission_percent ?? 5)
                    );
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono text-xs text-white/70">
              {selectedBranch?.qr_code_id.slice(0, 8)}...
            </span>
          </div>
        </CardContent>
      </Card>
      <AlertDialog
        open={showCommissionConfirm}
        onOpenChange={setShowCommissionConfirm}
      >
        <AlertDialogContent className="bg-background border-white/10">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={savingCommission}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              disabled={savingCommission}
                ...
                setBranches((prev) =>
                  prev.map((b) =>
                    b.id === selectedBranch!.id
                      ? { ...b, commission_percent: val }
                      : b
                  )
                );
                setSelectedBranch((prev) =>
                  prev ? { ...prev, commission_percent: val } : prev
                );
                setEditingCommission(false);
                setShowCommissionConfirm(false);
                toast({
                  title: "Updated",
                  description: `Commission rate set to ${val}%`,
                });
              }}
            >
              {savingCommission ? (
  ...
  const dialogsBlock = (
    <>
      ...
      <Dialog
        open={!!showQrDisplay}
        onOpenChange={() => setShowQrDisplay(null)}
      >
        <DialogContent className="max-w-xs bg-primary border-white/10">
          ...
          <div className="flex flex-col items-center py-4">
            ...
            <p className="mt-3 font-display text-lg font-bold text-secondary">
              {showQrDisplay?.label}
            </p>
            <p className="text-xs text-white/40 mt-1">
              {showQrDisplay?.type === "static"
                ? "Customer scans and enters amount"
                : "Amount is pre-filled for customer"}
            </p>
            <div className="flex gap-2 mt-4">
              <Button
                ...
                variant="outline"
                onClick={downloadQr}
                className="gap-1.5 border-white/10 text-white/70 hover:bg-white/10 hover:text-white"
              >
