...
import {
  getCached,
  setCached,
  invalidate as invalidateReferralCache,
...
const Referral = () => {
  ...
  const [tierCountsFromRpc, setTierCountsFromRpc] = useState<
  ...
  const fetchReferralsFromProfiles = useCallback(
    async (rootProfileId: string): Promise<ReferralMember[]> => {
      ...
      const tier1Ids = collected
        .filter((row) => row.tier === 1)
        .map((row) => row.user_id);
      let emailMap = new Map<string, string>();
      if (tier1Ids.length > 0) {
        const { data: emailsData } = await supabase.rpc("get_referral_emails", {
          referral_user_ids: tier1Ids,
        });
        emailMap = new Map(
          (
            (emailsData as { user_id: string; email: string }[] | null) || []
  ...
  const fetchAllEarnings = useCallback(
    async (userId: string): Promise<CommissionTx[]> => {
    ...
    async (opts?: { forceRefresh?: boolean; silent?: boolean }) => {
      ...
      try {
        ...
        if (profileRes.data) {
          ...
          const profileReferrals = await fetchReferralsFromProfiles(
            profileRes.data.id,
          );

          const derivedTierCounts = profileReferrals.reduce<
            Record<number, number>
          >((acc, row) => {
          ...
          const beyond =
            deepCountRes.data &&
            Array.isArray(deepCountRes.data) &&
            deepCountRes.data.length > 0
          ...
          const prevTotal = cached
            ? Object.values(cached.tierCounts).reduce((s, c) => s + c, 0) +
              cached.beyondTier5Count
            : null;
          const nextDirect = derivedTierCounts[1] || 0;
          const nextTotal =
            Object.values(derivedTierCounts).reduce((s, c) => s + c, 0) +
            beyond;
          const counts_changed =
            prevTotal !== null &&
            (prevDirect !== nextDirect || prevTotal !== nextTotal);
        ...
        // Fetch affiliate product click stats
        ...
        if (affiliateRows) {
          setAffiliateStats({
            totalClicks: affiliateRows.length,
            convertedOrders: affiliateRows.filter(
              (r: { converted: boolean; commission_earned: number }) =>
                r.converted,
            ).length,
            commissionEarned: affiliateRows.reduce(
              (
                sum: number,
                r: { converted: boolean; commission_earned: number },
        ...
        const message =
          err?.message ||
          "Couldn't refresh your network. Check your connection.";
  ...
  const handleRecount = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Direct: ${direct} • Network: ${total}`,
      });
    } catch (err) {
  ...
  const getShareUrl = () =>
    `https://nocap.life/auth?ref=${profile?.referral_code || ""}`;
  const getShareText = () =>
    `Join NOcap and earn cashback! Use my referral code *${profile?.referral_code}* to sign up: ${getShareUrl()}`;
  ...
  const generateQrImage = async (): Promise<File | null> => {
    try {
      ...
      return new File(
        [blob],
        `nocap-referral-${profile?.referral_code || "code"}.png`,
        { type: "image/png" },
      );
    } catch (e) {
  ...
  const shareToTelegram = () => {
    window.open(
      `https://t.me/share/url?url=${encodeURIComponent(getShareUrl())}&text=${encodeURIComponent(`Join NOcap! Use my code ${profile?.referral_code}`)}`,
      "_blank",
    );
  };
  ...
  const shareToTwitter = () => {
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Join NOcap and earn cashback! Use my referral code ${profile?.referral_code}`)}&url=${encodeURIComponent(getShareUrl())}`,
      "_blank",
    );
  };
  ...
  const totalNetworkCount =
    (Object.values(tierCountsFromRpc).reduce((s, c) => s + c, 0) ||
      referrals.length) + beyondTier5Count;

  // Per-type earnings breakdown
  const earningsByType = useMemo(() => {
    const cashback = commissions
      ...
      <div className="px-4 pb-14 pt-8">
        <div className="mx-auto max-w-md">
          <div className="flex items-center justify-between gap-3">
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={directTone}>
                        direct {fmt(lastDiff.direct)}
                      </span>
                      <span className="text-white/30">·</span>
                      <span className={totalTone}>
                        total {fmt(lastDiff.total)}
                      </span>
                    </span>
                  );
                })()}
            ...
            >
              <RefreshCw
                className={`h-3.5 w-3.5 ${recountLoading ? "animate-spin" : ""}`}
              />
              <span className="text-xs">
                {recountLoading ? "Refreshing..." : "Refresh"}
              </span>
            </Button>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-md px-4">
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              <div>
                ...
                <p className="mt-1 font-display text-xl font-bold text-white">
                  {tierCountsFromRpc[1] ||
                    referrals.filter((r) => r.tier === 1).length}
                </p>
              ...
              <div>
                ...
                <p className="mt-1 font-display text-xl font-bold text-white">
                  RM {earningsByType.cashback.toFixed(2)}
                </p>
            ...
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  ...
                  <p className="text-[10px] text-white/40">Total Clicks</p>
                </div>
                <div>
                  <p className="font-display text-lg font-bold text-secondary tabular-nums">
                    {affiliateStats.convertedOrders}
                  </p>
                  <p className="text-[10px] text-white/40">Converted</p>
                </div>
                <div>
                  <p className="font-display text-lg font-bold text-secondary tabular-nums">
                    RM {affiliateStats.commissionEarned.toFixed(2)}
                  </p>
                  <p className="text-[10px] text-white/40">Commission</p>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-4 flex gap-2 flex-wrap justify-center">
                ...
                <Button
                  size="sm"
                  onClick={handleShare}
                  className="gap-1.5 bg-secondary text-primary hover:bg-secondary/90"
                >
                ...
                {(earningsByType.commission > 0 || thisMonthCommission > 0) && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => setShowEarningsCard(true)}
                    className="gap-1.5 border-secondary/30 text-secondary hover:bg-secondary hover:text-primary"
                  >
        ...
        <Tabs defaultValue="tree" className="mt-6">
          <TabsList className="w-full grid grid-cols-2 bg-white/5 border border-white/10">
            <TabsTrigger
              value="tree"
              className="gap-1.5 data-[state=active]:bg-secondary data-[state=active]:text-primary text-white/50"
            >
                ...
                <CardContent className="p-3">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="text-center border-l border-white/10">
                      <p className="text-[10px] text-white/40 uppercase tracking-wider">
                        {new Date().toLocaleString("en-MY", { month: "long" })}
                      </p>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[11px] text-white/50 leading-relaxed">
                      Re-tally direct &amp; total network using paginated logic
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                      <div className="rounded-lg bg-white/5 p-2.5">
                        ...
                        <p className="font-display text-lg font-bold text-white tabular-nums">
                          {recountResult
                            ? recountResult.direct
                            : tierCountsFromRpc[1] || 0}
                        </p>
                      </div>
                      <div className="rounded-lg bg-white/5 p-2.5">
                        <p className="text-[10px] uppercase tracking-wide text-white/40">
                    ...
                    {recountResult && (
                      <div className="mt-2 flex items-center gap-1.5 text-[10px] text-secondary/80">
                        ...
                        <span>
                          Recounted at {recountResult.at.toLocaleTimeString()}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {tierCounts.map(({ tier, count }) => {
                  ...
                  return (
                    <div key={tier} className="flex items-center gap-3">
                      ...
                      <span className="text-xs tabular-nums text-white/50 w-6 text-right">
                        {count}
                      </span>
                    </div>
                  );
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-y border-white/10 bg-white/5">
                        <th className="px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-white/50">
                          Tier
                        </th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-white/50">
                          Label
                        </th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-white/50">
                          Members
                        </th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-white/50">
                          % of Total
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-2.5">
                              <span
                                className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${colors.bg} ${colors.text}`}
                              >
                                T{tier}
                              </span>
                            </td>
                      ...
                      {beyondTier5Count > 0 && (
                        <tr className="border-b border-white/5">
                          ...
                          <td className="px-4 py-2.5 text-right text-xs tabular-nums text-white/50">
                            {totalNetworkCount > 0
                              ? (
                                  (beyondTier5Count / totalNetworkCount) *
                                  100
                                ).toFixed(1)
                              : "0.0"}
                            %
                    ...
                    <tfoot>
                      <tr className="border-t border-white/10 bg-secondary/10">
                        <td
                          colSpan={2}
                          className="px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-secondary"
                        >
                          Total Network
                        </td>
                      ...
                      <tr className="border-t border-white/5 bg-white/5">
                        <td
                          colSpan={2}
                          className="px-4 py-2 text-[11px] text-white/50"
                        >
                          Direct (Tier 1)
                        </td>
                      ...
                      <tr className="bg-white/5">
                        ...
                        <td className="px-4 py-2 text-right text-xs font-semibold tabular-nums text-white/70">
                          {(
                            totalNetworkCount - (tierCounts[0]?.count || 0)
                          ).toLocaleString()}
                        </td>
                        <td className="px-4 py-2 text-right text-[11px] tabular-nums text-white/40">
                          {totalNetworkCount > 0
              ...
              <div className="relative">
                ...
                <div className="flex items-center gap-3 mb-2 px-1">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40">
                      Root • {totalNetworkCount} in network
                    </p>
                  </div>
                </div>
                ...
                {[1, 2, 3, 4, 5].map((tier) => {
                  ...
                  return (
                    <div key={tier} className="relative ml-5">
                      ...
                      <div
                        className={`absolute left-0 top-0 h-full w-px border-l-2 border-dashed ${colors.line}`}
                      />
                      ...
                      <div className="relative pl-6">
                        ...
                        <div
                          className={`absolute left-0 top-1/2 w-5 border-t-2 border-dashed ${colors.line}`}
                        />
                        {/* Branch dot */}
                        <div
                          className={`absolute left-[-5px] top-1/2 -translate-y-1/2 h-2.5 w-2.5 rounded-full ${colors.dot} ring-2 ring-primary`}
                        />
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <div
                              className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold ${colors.bg} ${colors.text}`}
                            >
                              T{tier}
                            </div>
                            <div className="min-w-0">
                              ...
                              {earningsByTier[tier] > 0 && (
                                <span
                                  className={`ml-2 text-[10px] font-semibold ${colors.text}`}
                                >
                              ...
                              {earningsByTier[tier] === 0 && (
                                <span className="ml-2 text-[10px] text-white/30">
                                  {tierCommissionLabels[tier]} per txn
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="relative pl-6 pb-1">
                          {members.map((member, idx) => (
                            <div key={member.user_id} className="relative">
                              ...
                              <div
                                className={`absolute left-0 top-0 w-4 border-t border-dashed ${colors.line}`}
                                style={{ top: "50%" }}
                              />
                              {idx < members.length - 1 && (
                                <div
                                  className={`absolute left-0 top-0 h-full w-px border-l border-dashed ${colors.line}`}
                                />
                              )}
                              {idx === members.length - 1 && (
                                <div
                                  className={`absolute left-0 top-0 h-1/2 w-px border-l border-dashed ${colors.line}`}
                                />
                              )}
                              {/* Small dot */}
                              <div
                                className={`absolute left-[-3px] top-1/2 -translate-y-1/2 h-1.5 w-1.5 rounded-full ${colors.dot}`}
                              />

                              <div className="flex items-center gap-3 pl-5 py-2">
                                <div
                                  className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${colors.bg} text-[10px] font-semibold ${colors.text}`}
                                >
                                  {(member.full_name || "?")
                                    .charAt(0)
                                    .toUpperCase()}
          ...
          <TabsContent value="earnings" className="mt-4 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="border-t border-white/10 pt-2 mt-2">
                    <p className="text-[10px] text-white/40 mb-2">
                      Distribution (from merchant commission pool):
                    </p>
                    {[
                      ...
                      const colors =
                        i === 0 ? tierColors[1] : tierColors[Math.min(i, 5)];
                      return (
                        <div
                          key={i}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className={`h-2 w-2 rounded-full ${i === 0 ? "bg-secondary" : colors.dot}`}
                            />
                  ...
                  <p className="text-[10px] text-white/30 mt-1 leading-relaxed">
                    Unclaimed tier commissions (no referrer at that level) are
            ...
            {/* Transaction History */}
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider pt-1">
              Recent Earnings
            </p>
            {commissions.length === 0 ? (
              <Card className="border-white/10 bg-white/5">
                <CardContent className="flex flex-col items-center justify-center py-10 text-white/40">
                  ...
                  <p className="mt-1 text-xs">
                    Commissions from your network will appear here
                  </p>
                </CardContent>
              </Card>
            ) : (
              commissions.map((tx) => (
                <Card key={tx.id} className="border-white/10 bg-white/5">
                  <CardContent className="flex items-center gap-3 p-3">
                    <div
                      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${tx.type === "cashback" ? "bg-secondary/20" : "bg-blue-500/20"}`}
                    >
                      {tx.type === "cashback" ? (
                    ...
                    <p
                      className={`text-sm font-semibold tabular-nums ${tx.type === "cashback" ? "text-secondary" : "text-blue-400"}`}
                    >
