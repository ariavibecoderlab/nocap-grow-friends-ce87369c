...
const DistributionAudit = () => {
  ...
  const load = async () => {
    ...
    const memberIds = Array.from(
      new Set(distRows.map((row) => row.metadata?.member_id).filter((id): id is string => Boolean(id))),
    );
    const branchIds = Array.from(
      new Set(distRows.map((row) => row.metadata?.branch_id).filter((id): id is string => Boolean(id))),
    );
    ...
    const [profilesRes, branchesRes, childrenRes] = await Promise.all([
      ...
      distributionIds.length
        ? supabase.from("transactions").select("reference_id, amount").in("reference_id", distributionIds)
        : Promise.resolve({ data: [] as ChildTx[], error: null }),
    ]);

    if (profilesRes.error || branchesRes.error || childrenRes.error) {
      setErrorMsg(profilesRes.error?.message || branchesRes.error?.message || childrenRes.error?.message || "Failed to load distribution audit data");
      setRows([]);
      setLoading(false);
      return;
    }
    ...
    const branchesMap = new Map((branchesRes.data ?? []).map((branch) => [branch.id, branch.branch_name]));
    const childSummaryMap = new Map<string, { child_count: number; child_total: number }>();

    for (const child of (childrenRes.data ?? []) as ChildTx[]) {
      if (!child.reference_id) continue;
      const current = childSummaryMap.get(child.reference_id) ?? { child_count: 0, child_total: 0 };
    ...
    const nextRows: Row[] = distRows.map((row) => {
      ...
      const childSummary = childSummaryMap.get(row.id) ?? { child_count: 0, child_total: 0 };
      ...
      return {
        ...
        branch_name: row.metadata?.branch_name ?? (branchId ? branchesMap.get(branchId) ?? null : null),
        ...
        reconciled: Math.abs(Number(row.amount || 0) - childSummary.child_total) < 0.001,
      };
    });
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.id, row.branch_name, row.member_name, row.member_referral_code, row.source, row.status]
        .filter(Boolean)
